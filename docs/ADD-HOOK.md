...
The generated hook command is the **extensionless** path `scripts/<name>`. A POSIX shell runs the executable `.sh`; `cmd.exe` appends `PATHEXT` and finds the generated `scripts/<name>.cmd`, which calls the `.ps1`. Codex is the only client with a real per-OS field, so it gets `commandWindows` instead of going through the shim.
