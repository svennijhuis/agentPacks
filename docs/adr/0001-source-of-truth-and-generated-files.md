...
MCP adapters, and skill-standard references. Claude and Copilot catalog entries point at the real
plugin directory. The Cursor catalog is the official thin name/source/description list (no `./`
prefix, no extra entry fields); component routing lives on each plugin's `.cursor-plugin/plugin.json`.
