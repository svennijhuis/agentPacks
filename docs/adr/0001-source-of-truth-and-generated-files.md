...
**Automation owns a separate generated branch.** Pull requests validate `main`, which contains no Claude marketplace or materialized external skills. A job on `main` publishes the complete installable tree to `distribution`.
...
- The generated distribution must never be hand-edited. Drift checks run against the `distribution` branch.
