...
7. Resolve the product-diff pin (run files omitted). Empty or bad pin → stop before any reviewer spawn. Directly launch `squad-reviewer`, `squad-simplifier`, and the conditional `squad-security-reviewer` in parallel against the product diff (run files omitted). Record why the security gate ran or was skipped.
