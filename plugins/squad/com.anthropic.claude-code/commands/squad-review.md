...
by the diff. Security only when the review contract's Security gate says run.
...
1. Resolve the diff under review:
   ...
   Pin the product diff. Resolve the product-diff pin: omit run files from the reviewer payload
   (review contract) only when they appear in the diff, and list those under Not examined.
   Product docs that are the work stay in. A missing run file is not a finding.
   Empty or bad pin → stop before any reviewer spawn.
   If the product diff is empty, say so and stop before any reviewer spawn.
2. Apply the review contract's Security gate: run `squad-security-reviewer` when the product diff touches auth, untrusted input, files, shell, crypto, dependencies, credentials, or exceptional conditions; otherwise skip. Record that reason. Do not launch a full-repo audit from this review.
...
6. Append one learnings entry. If this run evidenced a pack/skill/agent/command/contract change, append one suggestions entry. Do not apply it.
