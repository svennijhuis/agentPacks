...
5. Prove the plan's test-plan matrix. Deployed/API: prove pack `references/smoke-matrix.md` (happy/edge/fail/auth/timeout/5xx). Name behavioral + edge coverage. Reject happy-path-only coverage. Do not edit a test to make it pass. Do not edit source. Return the verifier report. Do not commit.

Good: `not verified` + "no command covers criterion 3".
Bad: mark `pass` because the happy-path test is green.
Good: Coverage names the empty-input and duplicate-id cases.
Bad: Coverage says `happy-path only` while the suite is green.
