...
- Drop findings whose location is a run file, and findings that a run file is missing, except synthesized verifier rows that use the plan path as Location (review contract).
...
Bad: assign `pass` while a criterion is `not verified` or `blocked`; input-error after a marker was supplied for that axis; invent Fix-list severity from a malformed-marker or handoff prose; launch a reviewer from this agent.
