...
4. Deduplicate on location + cause. Rank by severity. Apply the evaluator gates. A `fail` or `not verified` row blocks `pass`. Planned loop: append the merge report to the plan. `/squad-review`: standalone merge, no verdict.
