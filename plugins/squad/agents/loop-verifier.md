...
2. Commands from the plan, or every applicable stack's `<lang>-test-patterns` by exact Skill tool name. Say which source.
...
5. Reject happy-path-only coverage. Do not edit a test to make it pass. Return the verifier report. Do not commit.
